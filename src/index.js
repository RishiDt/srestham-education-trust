...
io.on('connection', (socket) => {
  console.log('A client connected SID & connectedSenders is', socket.id,connectedSenders);
  ...
  socket.on('locationRequest', (data) => {
    ...
    if (connectedSenders.has(senderId.toString())) {
      ...
      console.log("sender: ",senderId, "detected in connectedSenders. sending sendLocation event to it.");
      ...
      console.log("sender:" ,senderId, "not in connectedSenders. sending message to receiver:",receiverId);
      io.to(socket.id).emit('serverMessage', { 'message': 'sender is yet to connect','code':'NO_RECEIVER_CONNECTED' });
    }
  })
